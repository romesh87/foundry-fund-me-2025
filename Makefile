...
DEFAULT_SEPOLIA_ACCOUNT := metamaskAcc1

build:; forge build

deploy-sepolia:
	forge script script/DeployFundMe.s.sol --rpc-url $(SEPOLIA_RPC_URL) --account $(DEFAULT_SEPOLIA_ACCOUNT) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY)  -vvvv

# -include .env

# .PHONY: all test clean deploy fund help install snapshot format anvil zktest

# DEFAULT_ANVIL_KEY := defaultKey
# DEFAULT_ZKSYNC_LOCAL_KEY := zksyncKey
...
# NETWORK_ARGS := --rpc-url http://localhost:8545 --account $(DEFAULT_ANVIL_KEY) --broadcast
...
# deploy-zk:
# 	forge create src/FundMe.sol:FundMe --rpc-url http://127.0.0.1:8011 --account $(DEFAULT_ZKSYNC_LOCAL_KEY) --constructor-args $(shell forge create test/mock/MockV3Aggregator.sol:MockV3Aggregator --rpc-url http://127.0.0.1:8011 --account $(DEFAULT_ZKSYNC_LOCAL_KEY) --constructor-args 8 200000000000 --legacy --zksync | grep "Deployed to:" | awk '{print $$3}') --legacy --zksync
